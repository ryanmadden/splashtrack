  ...
  recordHit: function(gameId, playerId, recordType, rebuttalMode) {
    ...
    if (rebuttalMode) {
      ...
      if (opposingScore > 0) {
        ...
        alert("You can't hit a rebuttal unless the other team has points.");
      }
    }
    else {
      Games.update(gameId, {
    ...
    if (currGame !== null) {
      if (rebuttalMode) {
        if (currGame.records[playerId]['rebut' + recordType] !== null && currGame.records[playerId]['rebut' + recordType] > 0) {
          ...
          alert("Sorry, you can't subtract rebuttal " + recordType + " if the player doesn't have any.")
        }
      }
      else {
        if (currGame.records[playerId][recordType] !== null && currGame.records[playerId][recordType] > currGame.records[playerId]['rebut' + recordType]) {
          ...
          alert("Sorry, you can't subtract " + recordType + " if the player doesn't have any (or they only have rebuttals).");
        }
      }
    }
    else {
