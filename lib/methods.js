Meteor.methods({
  ...
  processAllRatedGames: function() {
    ...
    for (var x = 0; x < ratedGames.length; x++) {
      var result = Meteor.call('updateRatings', ratedGames[x]._id);
    }
  },
  recalculateRatings: function() {
    Meteor.call('resetAllRatings');
    Meteor.call('setAllRatings', 800);
    Meteor.call('processAllRatedGames');
  },
  ...
  updateStats: function(playerId) {
    ...
    else if (numGames < 100) { rank = "Elder Capsman"; }
