...
Template.players.helpers({
  ...
  playersRating: function () {
    return Meteor.users.find({'profile.stats.ratedGames': {$gte: 10}}, {sort: {'profile.rating': -1, 'profile.name': 1}}).fetch();
  },
  playersGames: function () {
    return Meteor.users.find({}, {sort: {'profile.stats.games': -1, 'profile.name': 1}}).fetch();
  },
  playersWinPct: function() {
    return Meteor.users.find({'profile.stats.games': {$gte: 10}}, {sort: {'profile.stats.winratio': -1, 'profile.name': 1}}).fetch();
  }
});
