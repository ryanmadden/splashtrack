Meteor.methods({
  ...
  updateRatings: function(gameId) {
    ...
    for (var i = 0; i < 4; i++) {
      if (i < 2) {
        newRatings[i] = oldRatings[i] + kValue * (homeResult - expectedHomeResult);
      }
      else {
        newRatings[i] = oldRatings[i] + kValue * (awayResult - expectedAwayResult);
      }
  ...
  updateStats: function(playerId) {
    ...
    var stats = {
      ...
      cpg: Math.round(allhits/games.length*100)/100 || 0,
      hits: hits,
      misses: misses,
      glass: glass,
      ...
      rpg: Math.round(allrebuttals/games.length*100)/100 || 0,
      rebuthits: rHits,
      rebutmisses: rMisses,
      rebutglass: rGlass,
